...
export default function Landing() {
  ...
  const onSend = async (override?: string) => {
    ...
    if (mode === "Quiz") {
      router.push(`/quiz?topic=${encodeURIComponent(q)}`);
      return;
    }
    ...
    try {
      if (stagedFile) {
        // Upload file to /api/upload, then redirect to chat with the doc_id for RAG
        const doc = await uploadDocument(stagedFile);
        const chromaId = `doc_${doc.id}`;
        window.dispatchEvent(new Event("chat:created"));
        const newChatId = crypto.randomUUID();
        router.push(
          `/chat?chatId=${encodeURIComponent(newChatId)}&q=${encodeURIComponent(q)}&doc_id=${encodeURIComponent(chromaId)}&doc_title=${encodeURIComponent(doc.title || doc.original_name || doc.filename)}`
        );
        return;
      }
      window.dispatchEvent(new Event("chat:created"));
      const newChatId = crypto.randomUUID();
      router.push(`/chat?chatId=${encodeURIComponent(newChatId)}&q=${encodeURIComponent(q)}`);
    } catch (err: any) {
