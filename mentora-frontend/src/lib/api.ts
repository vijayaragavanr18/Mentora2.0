...
export type ChatStartResponse = { ok: true; chatId?: string; chat_id: string; stream: string };
...
export async function chatAskOnce(opts: {
  ...
  const start = files.length ? await chatMultipart(q, files, finalChatId) : await chatJSON({ q, chatId: finalChatId });
  let answer = "";
  let flashcards: FlashCard[] | undefined;

  await new Promise<void>((resolve, reject) => {
    const { close } = connectChatStream(finalChatId, (ev) => {
      onEvent?.(ev);
      if (ev.type === "answer") {
        const p = ev.answer;
        if (typeof p === "string") {
  ...
  return { chatId: finalChatId, answer, flashcards };
}
...
export async function uploadDocument(
  ...
  return req<UploadedDocument>(`${env.backend}/api/upload`, {
    method: "POST",
    body: form,
    timeout: 120_000,
  });
...
export async function listDocuments(): Promise<UploadedDocument[]> {
  return req<UploadedDocument[]>(`${env.backend}/api/documents`);
}

export async function deleteDocument(docId: string): Promise<void> {
  await req<void>(`${env.backend}/api/documents/${encodeURIComponent(docId)}`, {
    method: "DELETE",
  });
}
