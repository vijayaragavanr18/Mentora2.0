...
type Props = {
  ...
  onUploadClick?: () => void;
  /** Number of active source docs (multi-doc RAG) */
  activeDocCount?: number;
  /** Legacy single-doc title — kept for compatibility */
  activeDocTitle?: string | null;
};

export default function Composer({ disabled, onSend, onUploadClick, activeDocCount = 0, activeDocTitle }: Props) {
  const inputRef = useRef<HTMLTextAreaElement>(null);
  const hasSource = activeDocCount > 0 || !!activeDocTitle;
  ...
  const sourceLabel = activeDocCount > 1
    ? `Searching ${activeDocCount} sources`
    : activeDocCount === 1
      ? "1 source active"
      : activeDocTitle
        ? activeDocTitle
  ...
  return (
    <div className="fixed bottom-0 pt-4 pb-4 border-t border-stone-900 left-4 right-4 lg:left-32 lg:right-4 z-40 bg-black">
      {/* Active sources indicator */}
      {sourceLabel && (
        <div className="max-w-4xl mx-auto mb-2 px-1">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-950/60 border border-sky-800 text-sky-300 text-xs">
            <svg xmlns="http://www.w3.org/2000/svg" className="size-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="relative rounded-3xl bg-stone-950 border border-stone-900 shadow-[inset_0_3px_15px] shadow-stone-900 flex items-end backdrop-blur-lg">
          {/* Sources / attach button */}
          <button
            type="button"
            onClick={onUploadClick}
            title={hasSource ? "Manage sources" : "Add sources"}
            className={`rounded-full p-2.5 transition-colors duration-200 m-2 flex-shrink-0
              ${hasSource
          ...
          <textarea
            ref={inputRef}
            placeholder={hasSource ? "Ask about your sources…" : "Ask a follow-up question or request more examples..."}
            rows={1}
