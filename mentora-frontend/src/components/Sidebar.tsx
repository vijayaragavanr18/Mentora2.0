...
export default function Sidebar() {
  const p = { pathname: usePathname() }.pathname
  const [isToggled, setIsToggled] = useState(false)
  const [chats, setChats] = useState<ChatsResponse | null>(null);
  const [sourcesState, setSourcesState] = useState<{ open: boolean; count: number }>({ open: false, count: 0 });

  useEffect(() => {
    const h = (e: Event) => {
      const d = (e as CustomEvent).detail;
      setSourcesState({ open: d.open, count: d.count });
    };
    window.addEventListener("sources:ui-state", h);
    return () => window.removeEventListener("sources:ui-state", h);
  }, []);
  ...
  const sourcesButtonClass = `p-2 rounded-xl duration-300 transition-all relative ${
    (p === '/chat' && sourcesState.open) ? 'text-white bg-stone-800' : 'hover:bg-stone-900 hover:text-stone-200'
  }`
  function toggleC() {
    ...
    <aside className='left-0 bottom-0 w-screen md:w-fit md:h-screen fixed p-4 z-50 lg:flex'>
      <div id='idk' className='w-64 md:order-2 h-full z-40 rounded-l-none rounded-2xl bg-stone-950 border border-l-transparent border-stone-900 hidden flex-col p-4 space-y-3 overflow-y-auto custom-scroll'>
        {chats?.chats?.map((chat) => (
          <Link key={chat.id} href={`/chat?chatId=${chat.id}`} className='p-2 hover:text-stone-200 hover:bg-stone-900 w-full rounded-xl block text-sm min-h-fit truncate'>
      ...
      <div id='idk0' className='md:w-20 md:order-1 h-full rounded-3xl bg-stone-950/50 backdrop-blur-xl border border-stone-900 text-stone-400 flex flex-col items-center justify-between py-4'>
        ...
        <nav className='flex md:flex-col items-center space-x-4 md:space-x-0 md:space-y-2 my-auto'>
          ...
          <Link href='/tools' className={b('/tools')}>
          ...
          <Link href='/planner' className={b('/planner')}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {/* Sources — opens the NotebookLM-style sources panel in /chat */}
          <button
            onClick={() => {
              if (p === '/chat') {
                window.dispatchEvent(new CustomEvent('sources:toggle'));
              } else {
                window.location.href = '/chat';
              }
            }}
            className={sourcesButtonClass}
            title="Sources"
          >
            <span className="relative flex items-center justify-center">
              ...
              {sourcesState.count > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-sky-500 text-white text-[9px] flex items-center justify-center font-bold leading-none">
                  {sourcesState.count}
                </span>
              )}
            </span>
